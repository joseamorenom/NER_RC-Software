...
def insert_tags(sentence, begin1, end1, begin2, end2):
    """Insert <e1>, </e1>, <e2>, </e2> tags at the correct positions in the sentence."""
    # Aseguramos que el primer argumento siempre sea el que tiene el inicio menor.
    if begin1 > begin2:
        begin1, end1, begin2, end2 = begin2, end2, begin1, end1

    # Desplazamiento acumulado inicial en 0
    offset = 0

    # Insertar <e1> y </e1> en la posición de begin1 y end1
    sentence = sentence[:begin1 + offset] + "<e1>" + sentence[begin1 + offset:end1 + offset] + "</e1>" + sentence[end1 + offset:]
    offset += len("<e1>") + len("</e1>")  # Actualizar el desplazamiento por la longitud de <e1> y </e1>

    # Ajustar posiciones de begin2 y end2 en función del desplazamiento acumulado
    begin2 += offset
    end2 += offset

    # Insertar <e2> y </e2> en la posición de begin2 y end2
    sentence = sentence[:begin2] + "<e2>" + sentence[begin2:end2] + "</e2>" + sentence[end2:]
...
def process_json_data(json_data):
    ...
    # Cargar las frases completas y las menciones
    ...
    if 'relations' in json_data:
        for relation in json_data['relations']:
            ...
            # Obtener la frase correspondiente a la relación
            sentence_id = args[0].split('-')[0]
            sentence_text = Frases_completas.get(sentence_id, "")
            original_sentence_text = sentence_text  # Guardar la frase original para referencia

            # Calcular la longitud de todas las oraciones previas dentro del mismo archivo
            previous_length = sum(len(Frases_completas[f"s{i}"]) for i in range(int(sentence_id[1:])))

            # Obtener y ajustar las posiciones de los argumentos
            arg1, arg2 = args
            begin1, end1 = Mentions[arg1]
            begin2, end2 = Mentions[arg2]

            # Ajustar las posiciones relativas dentro de la frase actual
            '''
            begin1 -= previous_length + 7
            end1 -= previous_length + 7
            begin2 -= previous_length + 7
            end2 -= previous_length + 7
            '''

            # Insertar las etiquetas <e1>, </e1>, <e2>, </e2> en las posiciones correctas
            sentence_text = insert_tags(sentence_text, begin1, end1, begin2, end2)

            # Agregar la frase con etiquetas al archivo de salida
            output_lines.append(f"{global_index}\t\"{sentence_text}\"\n")
...
#training_model_rc('Prueba_escritorio_guardar', r"C:\Users\amesa\OneDrive\GITA\2025\NER_RC\data\RC\prat2.json", r"C:\Users\amesa\OneDrive\GITA\2025\NER_RC\data\RC\rel2id_prat.json", 1)

use_model_rc('Test_Pratech', r"C:\Users\amesa\OneDrive\GITA\2025\NER_RC\data\RC\prat.json", r"C:\Users\amesa\OneDrive\GITA\2025\NER_RC\data\RC\salida_Prueba_escritorio_Prat.json")